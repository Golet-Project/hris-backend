...
func (m *Mobile) AddAttendance(ctx context.Context, req AddAttendanceIn) (out AddAttendanceOut) {
	// validate request
	...
	// validate user
	exist, err := m.db.CheckEmployeeById(ctx, domain, req.UID)
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
	...
	// input attendance
	err = m.db.AddAttendance(ctx, domain, db.AddAttendanceIn{
		EmployeeUID: req.UID,
