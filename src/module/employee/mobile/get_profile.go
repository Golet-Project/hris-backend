...
func (m *Mobile) GetProfile(ctx context.Context, in GetProfileIn) (out GetProfileOut) {
	// get user domain
	domain, err := m.userService.GetDomainByUid(ctx, in.UID)
	if err != nil {
	...
	employee, err := m.db.GetEmployeeDetail(ctx, domain, in.UID)
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
