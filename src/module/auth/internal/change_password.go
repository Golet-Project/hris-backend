...
func (i *Internal) ChangePassword(ctx context.Context, in ChangePasswordIn) (out ChangePasswordOut) {
	...
	rowsAffected, err := i.db.ChangePassword(ctx, db.ChangePasswordIn{
		UID:      in.UID,
	...
	err = i.redis.DeletePasswordRecoveryToken(ctx, in.UID)
	if err != nil && !errors.Is(err, redis.Nil) {
		out.SetResponse(http.StatusInternalServerError, "internal server error", err)
		return
	}
