...
func (i *Internal) ForgotPassword(ctx context.Context, in ForgotPasswordIn) (out ForgotPasswordOut) {
	...
	// check email
	admin, err := i.db.GetLoginCredential(ctx, in.Email)
	if err != nil {
	...
	existingToken, err := i.redis.GetPasswordRecoveryToken(ctx, admin.UserUID)
