...
func (m *Mobile) BasicAuthLogin(ctx context.Context, body BasicAuthLoginIn) (out BasicAuthLoginOut) {
	...
	// get the credentials
	loginCredential, err := m.db.GetLoginCredential(ctx, body.Email)
	if err != nil {
	...
	userDetail, err := m.db.GetEmployeeDetail(ctx, loginCredential.Domain, loginCredential.UserUID)
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
