...
func ValidateForgotPasswordRequest(req ForgotPasswordIn) *primitive.RequestValidationError {
	...
	} else if issues := validator.IsEmailValid(req.Email); len(issues) > 0 {
...
func (m *Mobile) ForgotPassword(ctx context.Context, req ForgotPasswordIn) (out ForgotPasswordOut) {
	...
	// get the login data
	user, err := m.db.GetLoginCredential(ctx, req.Email)
	if err != nil {
	...
	token, err := m.redis.GetPasswordRecoveryToken(ctx, user.UserUID)
	...
	err = m.redis.SetPasswordRecoveryToken(ctx, user.UserUID, token)
	if err != nil {
		out.SetResponse(http.StatusInternalServerError, "internal server error", err)
		return
	}
